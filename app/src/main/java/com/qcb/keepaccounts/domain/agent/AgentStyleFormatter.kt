...
class AgentStyleFormatter {
    ...
    fun formatWrite(
        ...
        if (facts.successCount > 0 && facts.failureCount > 0) {
            return listOf(
                "我先帮你处理好了 ${facts.successCount} 笔，还有 ${facts.failureCount} 笔需要补充一下信息。",
                "你点“去手动补全”就能快速修正，我会继续跟着你完成。",
            ).joinToString("\n\n")
        }

        val fallbackError = facts.errors.firstOrNull()?.takeIf { it.isNotBlank() }
            ?: "我这次没能成功处理。"
        return listOf(
            "这次还没处理成功。",
            fallbackError,
            "你可以换个说法再发一次，我会继续帮你跟进。",
        ).joinToString("\n\n")
    }
    ...
    private fun buildCreateSuccessLine(sceneHint: String): String {
        return when {
            ...
            sceneHint.contains("餐饮") || sceneHint.contains("晚饭") || sceneHint.contains("午饭") || sceneHint.contains("早餐") -> "收到，这笔吃饭开销已经帮你记上啦。"
    ...
    private fun buildCreateCareLine(sceneHint: String): String {
        return when {
            ...
            sceneHint.contains("餐饮") || sceneHint.contains("晚饭") || sceneHint.contains("午饭") || sceneHint.contains("早餐") -> "记得按时吃饭呀，要不要我顺便统计下这周餐饮花费？"
