...
5. PhaseC（解析能力补全）：已完成语义归一化组件落地。
- 新增 `AmountNormalizer`：支持中文金额（含口语如“一百五”）与基础加减表达式归一化。
- 新增 `CountNormalizer`：支持“前十二笔/12笔/两笔”等数量词归一化。
- 新增 `TemporalResolverV2`：支持“上周三”“上个月五号”等相对时间解析，并输出 confidence 与 trace。
6. PhaseC（对话体验修复）：已完成 5 个线上问题收口。
- 记账场景新增生活内容共情回应，避免“只记账不回应生活分享”。
- 备注提取规则升级，修复“中午吃饭”被错误记为“中午钱”的问题。
